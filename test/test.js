...
var closestX = require('..');


describe('closestX', function () {
    it('should yield the correct result with even element array', function () {
        var xs = [-1, 0, 1, 2, 3, 4, 5, 6];
        var ys = [10, 11, 12, 13, 14, 15, 16, 17];
        closestX(xs, ys, -2).should.eql({x: -1, y: 10});
        closestX(xs, ys, 0.6).should.eql({x: 1, y: 12});
        closestX(xs, ys, 4.3).should.eql({x: 4, y: 15});
        closestX(xs, ys, 6).should.eql({x: 6, y: 17});
        closestX(xs, ys, 7).should.eql({x: 6, y: 17});
    });
    ...
    it('should yield the correct result with odd element array', function () {
        ...
        closestX(xs, ys, 4.3).should.eql({x: 4, y: 15});
        closestX(xs, ys, 6).should.eql({x: 6, y: 17});
        closestX(xs, ys, 7).should.eql({x: 7, y: 18});
        closestX(xs, ys, 7).should.eql({x: 7, y: 18});
    });
